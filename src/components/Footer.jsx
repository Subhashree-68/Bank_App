  ...
  <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
    <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
      <div className='flex-[1] flex flex-col justify-start mr-10'>
        <img src={logo} alt="" className='w-[150px] h-[70px] object-contain' />
          ...
          <div key={footerLink.key} className='flex flex-col ss:my-0 my-4 min-w-[150px]'>
            ...
            <ul className='list-none mt-4'>
              {footerLink.links.map((Link, name) => (
                <li key={Link.name} className='font-poppins font-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer p-2 ${index!== footerlink.links.length-1?"mb-4" :"mb-0"}'>
                  {Link.name}
                </li>
              ))}
            </ul>
